...
import { canShowHuntRanking, isHuntOpen, type HuntEventRecord, type HuntRankingRow, type HuntSubmissionRecord } from "@/lib/hunt";
...
export default function HuntClient({
  ...
  const correctCount = submissions.filter((submission) => submission.status === "correct").length;
  return (
    <>
      ...
      <section className="hunt-intro">
        ...
        <article className="hunt-summary">
          ...
          <dl>
            ...
            <div><dt>你的正確數量</dt><dd>{correctCount}</dd></div>
            <div><dt>活動狀態</dt><dd>{open ? "進行中" : event.status === "results_published" ? "結果公布" : "未開放"}</dd></div>
          </dl>
          <p>每找到一個物品請上傳一張照片。相同照片或同一藏物重複提交只會計算一次。</p>
        </article>
      </section>
      ...
      <section className="hunt-upload-panel">
        ...
        {!userId ? (
          ...
          <form onSubmit={submit}>
            ...
            <button className="primary" disabled={busy || !file}>{busy ? "正在送出…" : "送出等待審核"}</button>
          </form>
        )}
