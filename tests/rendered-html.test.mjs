...
test("AI background originals are appended to the public entry gallery with a signed URL", async () => {
  ...
  assert.match(entryPage, /originalRevision\?\.storage_object_path \?\? entry\.original_image_path/);
  assert.match(entryPage, /\.from\("cos-originals"\)\.createSignedUrl\(activeOriginalPath,\s*60 \* 60\)/);
